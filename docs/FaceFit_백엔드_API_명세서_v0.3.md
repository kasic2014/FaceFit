...
> v0.3에서는 온보딩 구조와 음성 클론 `VOICE-001~003`을 유지합니다. 지원공고 입력은 2026-07-30 확정 정책에 따라 사용자가 직접 업로드하는 스크린샷 이미지 또는 HWP·PDF 파일만 허용합니다.
...
| 지원공고 입력 | 사용자가 제공한 `FILE` 또는 `TEXT` | 사용자가 직접 업로드한 JPG·JPEG·PNG 스크린샷 이미지 또는 HWP·PDF 파일 |
...
- 지원공고 JPG·JPEG·PNG·HWP·PDF 파일 검증과 비공개 저장
- 스크린샷 이미지 OCR 또는 HWP·PDF 텍스트 추출과 구조화 상태 관리
...
| 지원공고 입력 | JPG·JPEG·PNG·HWP·PDF 파일만 허용 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| JOB-001 | POST | `/api/v1/job-postings` | 지원공고 등록 | `multipart/form-data` | `file` | `jobPostingId`, `processingStatus` | 파일 검증·비공개 저장, 소유권 연결, OCR 또는 HWP·PDF 텍스트 추출·구조화 시작 |
...
| 공고문 파일 | HWP, PDF | 문서 파서로 텍스트를 추출한 뒤 지원정보 구조화 |

프론트 요청에서 `inputType`을 받지 않습니다. 백엔드는 검증된 파일의 MIME 타입과 실제 형식으로 OCR 또는 문서 파서를 선택하고, 내부 `JOB_POSTINGS.input_type`에는 호환성을 위해 `FILE`을 기록합니다.

URL, `rawText`, `TEXT` 붙여넣기 및 `SCRIPT` 입력은 받지 않습니다. 업로드된 파일에 HTML·JavaScript·매크로·명령문 등이 포함되어도 서버에서 실행하지 않습니다.
...
허용 확장자는 JPG, JPEG, PNG, HWP, PDF이며 지원공고 한 건당 파일 한 개를 업로드합니다. 최대 파일 크기, 이미지 해상도, PDF 페이지 수와 암호화·손상 파일 처리 기준은 구현 전에 확정합니다.
...
### 5.1 스크린샷 이미지·공고문 파일 입력

```text
JOB-001 스크린샷 이미지 또는 HWP·PDF 파일 업로드
→ 확장자·MIME 타입·실제 파일 형식 검증
→ 비공개 저장
→ processingStatus=PROCESSING
→ 이미지이면 OCR, HWP·PDF이면 문서 파서로 텍스트 추출
→ 지원정보 구조화
...
| 415 | `FILE_TYPE_NOT_SUPPORTED` | JPG·JPEG·PNG·HWP·PDF 이외의 파일 또는 확장자·MIME·실제 형식 불일치 |
...
4. 지원공고 스크린샷 이미지·HWP/PDF 파일: JOB-001~005
...
- 지원공고 파일 최대 용량
- 지원공고 이미지 최소 해상도와 최대 크기
- 지원공고 PDF 최대 페이지 수와 암호화 PDF 처리 기준
- 손상되거나 암호화된 HWP 처리 기준
