...
- 질문 단위 TTS+MuseTalk MP4와 프론트 반응형 캐릭터를 결합해 구현 가능한 질문·답변 흐름을 제공한다.
...
AI 직무 취업 준비생. 지원 대상은 `AI_ENGINEER`, `ML_ENGINEER`, `DATA_SCIENTIST`, `DATA_ENGINEER`, `MLOPS_ENGINEER`, `LLM_ENGINEER`, `NLP_ENGINEER`, `COMPUTER_VISION_ENGINEER`, `AI_PRODUCT_MANAGER`로 한정한다.
...
| ID | 요구사항 | 우선순위 | 수용 기준 |
| --- | --- | --- | --- |
| LIVE-01 | MuseTalk 캐릭터 MP4가 TTS 음성과 립싱크해 질문한다. | P0 | 질문 영상 또는 정적 폴백으로 5문항을 완료한다. |
| LIVE-02 | App API가 질문→TTS→MuseTalk 작업과 상태를 조정한다. | P0 | polling으로 준비·성공·폴백 상태가 추적된다. |
...
| LIVE-05 | 발화 후 3초부터 VAD를 시작하고 2초 무음 뒤 1초 카운트다운 동안 추가 발화가 없으면 턴을 종료한다. 버튼·Space는 항상 즉시 작동한다. | P0 | 카운트다운 중 재발화 시 자동 종료가 취소되고 `endedBy`가 정확히 저장된다. |
...
| 날짜 | 결정 | 이유 |
| --- | --- | --- |
| 2026-07-20 | React+Vite 웹 앱과 MuseTalk GPU worker를 분리하고, 기존 화면·행동·URL을 React Router SPA로 전환했다. | 브라우저 UI와 Python·CUDA 런타임을 독립 운영하기 위해서다. |
| 2026-07-31 | OpenAvatarChat·RTC를 제외하고 MuseTalk 질문 MP4+정적 폴백을 P0로 둔다. | 개발·운영 복잡도를 낮추면서 질문 몰입감을 유지하기 위해서다. |
