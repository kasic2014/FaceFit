...
| FL-11 침묵 감지 | 발화 3초 후·VAD 연결 | 2초 무음→1초 카운트다운→미발화 시 완료 | 재발화 시 자동 취소, 버튼·Space는 항상 유지 | 다음 턴 후보 | SC-008 | FR-018 |
...
stateDiagram-v2
  ...
  Recording --> Saving: 답변 완료·Space·VAD 확정
...
- VAD 임계값(3초 발화 후 감지, 2초 무음, 1초 카운트다운)은 API v0.3.6 기준으로 확정했다.
