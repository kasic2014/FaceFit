...
> **참조본 — 단독 구현 기준 사용 금지.** root [`../FaceFit_백엔드_API_명세서_v0.3.md`](../FaceFit_백엔드_API_명세서_v0.3.md)와 이 사본의 오류 envelope·API 목록·DTO가 다르다. 확정 전 기준과 해소 절차는 [API 계약 기준선](API_CONTRACT_BASELINE_2026-08-05.md)을 따른다.
...
| LEGAL-005 | GET | `/api/v1/legal-consents` | 내 기능별 동의 기록 목록 조회 |
...
모든 JSON 오류는 12.3의 flat envelope 형식인
`{success:false,code,message,data:{fieldErrors,retryable,retryAfterSec},requestId,timestamp}`을 사용하며,
아래 표 외 `code`를 프론트 계약에 추가하지 않는다.
...
| LEGAL-005 | GET | `/api/v1/legal-consents` |
...
| QUESTION-002 | path `sessionId`,`questionId` | 200 `PlaybackAccess`, 준비 중 202, 폴백 409 |
...
| VOICE-001 | multipart `consentLegalRecordId`, `file` | 202 `VoiceProfile` |
...
#### LEGAL-005 내 기능별 동의 기록 목록

`GET /api/v1/legal-consents?status=ACTIVE|REVOKED&type={LegalDocumentType}&sessionId={uuid}`

- 현재 로그인 회원의 동의 기록만 반환한다. 기본 `status=ACTIVE`, 페이지 기본값은 12.3을 따른다.
- `INTERVIEW_MEDIA_PROCESSING`은 `sessionId` 필터를 허용하고, 다른 문서 종류에 `sessionId`를 주면 `400 VALIDATION_FAILED`다.
- 응답은 `LegalConsentRecord`의 page이며, 프론트는 `ACTIVE` record의 `consentRecordId`로만 LEGAL-004 철회 요청을 보낸다.
- 이 API는 동의 철회 화면이 새로고침 후에도 본인 동의 상태를 복구하기 위해 추가한다.
...
- 음성 준비 중: HTTP 202 JSON 성공 envelope의 `data`는 `{status:"PROCESSING",retryAfterSec:3}`이다.
...
| `/source-resources` | 없음 | DOC-002, JOB-002 |
| `/sessions/:sessionId/settings` | `sessionId` | SESSION-002, DOC-002, JOB-002 |
