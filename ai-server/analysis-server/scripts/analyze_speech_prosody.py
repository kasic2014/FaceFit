"""Extract experimental numpy-based pitch, loudness, and intonation metrics."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("speech_metrics_json", type=Path)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument(
        "--include-frames",
        action="store_true",
        help="Include the potentially large per-frame array in output JSON.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = analyze_speech_prosody(
        args.wav_file,
        args.stt_json,
        args.speech_metrics_json,
